...
export function ResultsPage({ isDark, initialQuery, onLogoClick }: ResultsPageProps) {
  ...
  const results: SearchResult[] = state.status === "success" ? state.data.results : []
  const totalResults: number = state.status === "success" ? state.data.total : 0
  const latencyMs: number | null = state.status === "success" ? state.data.latency_ms : null
  ...
  return (
    <div className={cn("min-h-screen transition-colors duration-300", isDark ? "bg-dark-bg" : "bg-light-bg")}>
      {/* Header bar */}
      <div
        className={cn(
      ...
      >
        <div className="px-6 md:px-12 lg:px-24 py-4 flex items-center gap-8">
          ...
          >
            ...
            <span className={cn("text-2xl font-bold transition-colors", isDark ? "text-dark-text" : "text-light-text")}>
              Faro
            </span>
          </button>

          {/* Compact search bar */}
          <SearchInput value={query} onChange={setQuery} onSubmit={handleSearch} isDark={isDark} compact />
        </div>
      </div>
      ...
      <div className="py-5">
        <div className="px-6 md:px-12 lg:px-24 flex gap-8">
          ...
          <div className="flex-1 max-w-2xl">
            {/* Status line */}
            <SearchStatusLine state={state} totalResults={totalResults} latencyMs={latencyMs} isDark={isDark} />

            {/* Content */}
            {state.status === "loading" && <LoadingState isDark={isDark} />}
            {state.status === "success" && results.length === 0 && <EmptyState isDark={isDark} />}
            {state.status === "success" && results.length > 0 && (
              <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ duration: 0.2 }}>
                <ResultList results={paginatedResults} isDark={isDark} />
                <Pagination current={page} total={totalPages} onPageChange={handlePageChange} isDark={isDark} />
              </motion.div>
            )}
            {state.status === "error" && <ErrorState message={state.error} isDark={isDark} onRetry={handleSearch} />}
...
function ErrorState({ message, isDark, onRetry }: { message: string; isDark: boolean; onRetry: () => void }) {
  return (
    <div className="flex flex-col items-center py-16">
      <p className={`text-lg mb-2 ${isDark ? "text-dark-text-mid" : "text-light-text-mid"}`}>搜索出了点问题</p>
      <p className={`text-sm mb-4 ${isDark ? "text-dark-text-dim" : "text-light-text-mid"}`}>{message}</p>
      <button
        onClick={onRetry}
        className={cn(
